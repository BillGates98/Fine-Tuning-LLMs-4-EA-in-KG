...
class CSVMerger:
    def __init__(self, index, input_dir, output_file):
        self.index = index
        self.input_dir = input_dir
        self.output_file = output_file
        self.directories = ["KnowledgeGraphsDataSet", "Yago-Wiki", 'person', 'restaurant', 'anatomy', 'doremus',
                            'SPIMBENCH_small-2019', 'SPIMBENCH_large-2016']

    def merge(self):
        # Get a list of all the .csv files in the input directory
        csv_files = []
        for directory in self.directories[self.index:]:
...
if __name__ == "__main__":
    ...
    csv_merger = CSVMerger(index, input_dir, output_file)
    csv_merger.merge()
